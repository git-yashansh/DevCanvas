...
export function AIOrb({
  ...
  scale = 1,
  ...props
}: SiriWaveProps & { scale?: number }) {
  const canvasRef = React.useRef<HTMLCanvasElement>(null)

  const aspectHeight = variant === "wave" ? 0.45 : 1.0;
  const wDim = Math.round(size * renderScale);
  const hDim = Math.round(size * aspectHeight * renderScale);
