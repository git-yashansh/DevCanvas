...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    // Single static render pass to eliminate CPU/GPU animation thread rendering during scrolling
    const renderOnce = () => {
      if (!programRef.current || !rendererRef.current) return;
      programRef.current.uniforms.uMouse.value = mouseSmoothRef.current;
      programRef.current.uniforms.uTime.value = 8.0; // Use a constant time offset to render a beautiful static burst
    ...
    return () => {
      ro?.disconnect();
      if (!ro) (window as any).removeEventListener('resize', resize);
      try {
  ...
  useEffect(() => {
    ...
    // Trigger static re-render since uniforms changed
